...
<li class="breadcrumb-item active">Appointments</li>
                        ...
                        <div class="col-auto text-right">
                            <a href="type_list_form.php" class="btn btn-primary"> Add Types </a>
                        </div>
                    </div>
                </div>
                                ...
                                <div class="table-responsive">
                                    <table class="table">
                                        ...
                                        <tbody>
                                            ...
                                            $query = "SELECT * FROM  type where status='1'  order by id asc ";

                                            $exe = mysqli_query($conn, $query);
                                            $counter = 0;
                                            while ($row = mysqli_fetch_array($exe)) {
                                                $counter = $counter + 1;
                                                $catname = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM category where id='" . $row['catagory_id'] . "'"))

                                            ?>
                                                <tr>
                                                    <td><?php echo $counter; ?></td>
                                                    <td><?php echo $row['type'] ?></td>
                                                    <td><?php echo $catname['catagory_name'];  ?></td>


                                                    <td class="text-right d-flex">


                                                        <a href="type_list_delet.php?id=<?php echo $row['id']   ?>" class="btn btn-sm btn-white text-danger mr-2 d-flex"><i class="far fa-trash-alt mr-1"></i>Delete</a>
